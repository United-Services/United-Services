...
describe('KekKeyStore', () => {
  function makeStore() {
    const prisma = { kekRegistry: { findMany: jest.fn(), findUnique: jest.fn(), findFirst: jest.fn() } } as unknown as PrismaService;
    return { store: new KekKeyStore(prisma), prisma };
  }

  it('throws a clear error asking for a restart when a keyId was never loaded', () => {
    const { store } = makeStore();
    expect(() => store.getPrivateKey('kek-never-loaded')).toThrow(InternalServerErrorException);
  });
  ...
  it('throws when KEK_KEYS_DIR is not configured at all', async () => {
    ...
    (prisma.kekRegistry.findMany as jest.Mock).mockResolvedValue([{ keyId: 'kek-1', status: 'active' }]);

    await expect(store.onModuleInit()).rejects.toThrow(InternalServerErrorException);
  ...
  it('getPublicKey throws for an unknown keyId rather than returning undefined', async () => {
    ...
    await expect(store.getPublicKey('kek-unknown')).rejects.toThrow(InternalServerErrorException);
  });
