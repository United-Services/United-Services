...
describe('GeoController', () => {
  function makeController(localeReturn: string) {
    const geo = { localeForIp: jest.fn().mockReturnValue(localeReturn) } as unknown as GeoService;
  ...
  it('uses the first IP in a comma-separated X-Forwarded-For chain', () => {
    ...
    const req: any = { headers: { 'x-forwarded-for': '203.0.113.5, 10.0.0.1, 10.0.0.2' }, socket: { remoteAddress: '10.0.0.9' } };
