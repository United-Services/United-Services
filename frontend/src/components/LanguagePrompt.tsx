...
const COPY: Record<SwitchableLocale, { question: string; switchLabel: string; dismissLabel: string; dir: 'rtl' | 'ltr' }> = {
  ar: {
    question: 'يبدو أنك تزور الموقع من منطقة يُفضَّل فيها العربية. هل ترغب في التبديل إلى العربية؟',
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <button
          ...
          style={{ background: palette.accent, color: '#fff', border: 'none', borderRadius: 9999, padding: '9px 18px', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins, sans-serif' }}
        >
          {copy.switchLabel}
        </button>
        <button
          onClick={dismiss}
          style={{ background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 9999, padding: '9px 18px', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins, sans-serif' }}
        >
          {copy.dismissLabel}
        </button>
      </div>
