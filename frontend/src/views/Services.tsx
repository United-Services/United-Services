...
const LAYER_STYLE: Record<(typeof LAYER_KEYS)[number], { color: string; width: string }> = {
          ...
          <div className="reveal-left">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {LAYER_KEYS.map((key, i) => (
                ...
                >
                  <div style={{ width: 12, height: 12, borderRadius: 3, background: LAYER_STYLE[key].color, flexShrink: 0 }} />
                  <span style={{ fontSize: 13, color: palette.slate, fontWeight: 500 }}>{t(`diagram.layers.${key}.label` as any)}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                    ...
                    <div style={{ textAlign: 'start' }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: palette.navy }}>{tSvc(key)}</div>
                      <div style={{ fontSize: 12, color: palette.muted, marginTop: 2 }}>{t(`tags.${key}` as any)}</div>
                    </div>
                  </div>
                ...
                {active === i && (
                  <div style={{ background: '#FFFAF7', borderTop: '1px solid #FDE8D0', padding: '36px 32px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'start' }}>
                      <div>
                        <img src={SVC_IMGS[key]} alt={t(`imgAlt.${key}` as any)} style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover', borderRadius: 14, marginBottom: 24 }} />
                      ...
                      <div>
                        <p style={{ fontSize: 15, color: palette.slate, lineHeight: 1.8, marginBottom: 32 }}>{t(`desc.${key}` as any)}</p>
