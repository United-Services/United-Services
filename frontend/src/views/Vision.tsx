...
const PILLAR_KEYS = ['technical', 'regional', 'integrated', 'partnership'] as const
        ...
        <div style={{ maxWidth: 1260, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, marginBottom: 72 }}>
            <div className="reveal-left" style={{ background: palette.accentLight, border: `1px solid #FED7AA`, borderRadius: 20, padding: '48px 40px' }}>
              <div style={{ fontSize: 11, color: palette.accent, letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 20 }}>{t('visionLabel')}</div>
            ...
            <div className="reveal-right" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 20, padding: '48px 40px' }}>
              <div style={{ fontSize: 11, color: palette.accent, letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 20 }}>{t('missionLabel')}</div>
              <h2 style={{ fontSize: 28, fontWeight: 800, color: palette.navy, letterSpacing: '-0.02em', marginBottom: 20, lineHeight: 1.2 }}>
                {t('missionTitle')}
              </h2>
              <p style={{ fontSize: 15, color: palette.slate, lineHeight: 1.8 }}>
          ...
          <div className="reveal">
            <div style={{ fontSize: 11, color: palette.accent, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 40, textAlign: 'center' }}>{t('pillarsLabel')}</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
              {PILLAR_KEYS.map((key, i) => (
                <div key={key} className="reveal" style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: '28px 24px', transitionDelay: `${i * 0.08}s` }}>
                  <div style={{ fontSize: 36, fontWeight: 900, color: '#F1F5F9', lineHeight: 1, marginBottom: 16 }}>{String(i + 1).padStart(2, '0')}</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: palette.navy, marginBottom: 10 }}>{t(`pillars.${key}.title` as any)}</div>
                  <div style={{ fontSize: 13, color: palette.muted, lineHeight: 1.7 }}>{t(`pillars.${key}.desc` as any)}</div>
                </div>
              ))}
            </div>
          </div>
