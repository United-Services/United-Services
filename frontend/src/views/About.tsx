  ...
  return (
    <div style={{ fontFamily: 'Poppins, sans-serif', background: '#fff' }}>
      ...
      <section style={{ padding: '80px 28px' }}>
        <div style={{ maxWidth: 1260, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72, alignItems: 'center' }}>
          ...
          <div className="reveal-right">
            <div style={{ fontSize: 11, color: palette.accent, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 16 }}>{t('story.eyebrow')}</div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: palette.navy, letterSpacing: '-0.02em', marginBottom: 20 }}>{t('story.title')}</h2>
      ...
      <section style={{ background: '#F8FAFC', padding: '72px 28px' }}>
        <div style={{ maxWidth: 1260, margin: '0 auto' }}>
          <div className="reveal" style={{ textAlign: 'center', marginBottom: 56 }}>
            <div style={{ fontSize: 11, color: palette.accent, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 16 }}>{t('certs.eyebrow')}</div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: palette.navy, letterSpacing: '-0.02em' }}>{t('certs.title')}</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16 }}>
            {CERT_KEYS.map((key, i) => (
              <div key={key} className="reveal" style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: '28px 20px', textAlign: 'center', transitionDelay: `${i * 0.08}s` }}>
                <div style={{ fontSize: 22, fontWeight: 800, color: palette.accent, marginBottom: 8, letterSpacing: '-0.01em' }}>{CERT_CODES[key]}</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: palette.navy, marginBottom: 8 }}>{t(`certs.${key}.body` as any)}</div>
                <div style={{ fontSize: 11, color: palette.muted, lineHeight: 1.6 }}>{t(`certs.${key}.detail` as any)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '72px 28px' }}>
        <div style={{ maxWidth: 1260, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72, alignItems: 'center' }}>
          <div className="reveal-left">
            <div style={{ fontSize: 11, color: palette.accent, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 16 }}>{t('facility.eyebrow')}</div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: palette.navy, letterSpacing: '-0.02em', marginBottom: 20 }}>{t('facility.title')}</h2>
            <p style={{ fontSize: 15, color: palette.slate, lineHeight: 1.8, marginBottom: 20 }}>
              {t('facility.body')}
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 32 }}>
              {stats.map((s) => (
                <div key={s.label} style={{ background: '#F8FAFC', borderRadius: 14, padding: '20px 20px', border: '1px solid #E2E8F0' }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: palette.accent }}>{s.value}</div>
                  <div style={{ fontSize: 12, color: palette.muted, marginTop: 4 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
